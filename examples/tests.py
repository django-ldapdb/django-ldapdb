...
contacts = (
    'ou=contacts,ou=groups,dc=example,dc=org',
    {'objectClass': ['top', 'organizationalUnit'], 'ou': ['groups']},
)
rooms = (
    'ou=rooms,dc=example,dc=org',
    {'objectClass': ['top', 'organizationalUnit'], 'ou': ['rooms']},
)
foogroup = (
    'cn=foogroup,ou=groups,dc=example,dc=org',
    {
        'objectClass': ['posixGroup'],
        'memberUid': ['foouser', 'baruser'],
        'gidNumber': ['1000'],
        'cn': ['foogroup'],
    },
)
bargroup = (
    'cn=bargroup,ou=groups,dc=example,dc=org',
    {
        'objectClass': ['posixGroup'],
        'memberUid': ['zoouser', 'baruser'],
        'gidNumber': ['1001'],
        'cn': ['bargroup'],
    },
)
wizgroup = (
    'cn=wizgroup,ou=groups,dc=example,dc=org',
    {
        'objectClass': ['posixGroup'],
        'memberUid': ['wizuser', 'baruser'],
        'gidNumber': ['1002'],
        'cn': ['wizgroup'],
    },
)
foouser = (
    'uid=foouser,ou=people,dc=example,dc=org',
    {
...
class UserFactory(factory.django.DjangoModelFactory):
    ...
    password = factory.LazyAttribute(
        lambda o: auth_hashers.make_password(o.cleartext_password)
    )
...
class GroupTestCase(BaseTestCase):
    ...
    def test_annotate_count(self):
        groups = LdapGroup.objects.order_by('name').annotate(
            num_usernames=Count('usernames')
        )
        ...
        groups = LdapGroup.objects.filter(name='foogroup').annotate(
            num_usernames=Count('usernames')
        )
        self.assertEqual(len(groups), 1)
        self.assertEqual(groups[0].name, 'foogroup')
        self.assertEqual(groups[0].num_usernames, 2)
        groups = LdapGroup.objects.annotate(num_usernames=Count('usernames')).filter(
            name='foogroup'
        )
        self.assertEqual(len(groups), 1)
        self.assertEqual(groups[0].name, 'foogroup')
    ...
    def test_ldap_filter(self):
        def get_filterstr(qs):
            ...
            compiler = SQLCompiler(query=qs.query, connection=connection, using=None)
        ...
        self.assertEqual(
            get_filterstr(qs), '(&(objectClass=posixGroup)(!(cn=foogroup)))'
        )
    ...
    def test_get(self):
        ...
        self.assertRaises(
            LdapGroup.DoesNotExist, LdapGroup.objects.get, name='does_not_exist'
        )
    ...
    def test_create(self):
        LdapGroup.objects.create(
            name='newgroup', gid=1010, usernames=['someuser', 'foouser']
        )
    ...
    def test_paginated_search(self):
        ...
        self.assertIsNone(
            settings.DATABASES['ldap'].get('CONNECTION_OPTIONS', {}).get('page_size')
        )
        ...
        # Set new page size
        settings.DATABASES['ldap']['CONNECTION_OPTIONS'] = settings.DATABASES[
            'ldap'
...
class UserTestCase(BaseTestCase):
    ...
    def test_verbose_name(self):
        self.assertEqual(
            "Prime name", LdapUser._meta.get_field('first_name').verbose_name
        )
        self.assertEqual(
            "Final name", LdapUser._meta.get_field('last_name').verbose_name
        )
    ...
    def test_get(self):
        ...
        self.assertRaises(
            LdapUser.DoesNotExist, LdapUser.objects.get, username='does_not_exist'
        )
...
class CompositePKTests(BaseTestCase):
    ...
    def test_create(self):
        room = LdapMultiPKRoom(name="Director office", number="42.01")
        room.save()
        room = LdapMultiPKRoom.objects.get()
        self.assertEqual(
            "cn=Director office+roomNumber=42.01,ou=rooms,dc=example,dc=org", room.dn
        )
    ...
    def test_move(self):
        room = LdapMultiPKRoom.objects.create(name="Director office", number="42.01")
        ...
        self.assertEqual(
            "cn=Director office+roomNumber=42.02,ou=rooms,dc=example,dc=org",
            new_room.dn,
        )

    def test_update(self):
        room = LdapMultiPKRoom.objects.create(
            name="Director office", number="42.01", phone='+001234'
        )
        ...
        self.assertEqual(
            "cn=Director office+roomNumber=42.01,ou=rooms,dc=example,dc=org",
            new_room.dn,
        )
    ...
    def test_update_ambiguous_pk(self):
        ...
        LdapMultiPKRoom.objects.create(
            name="Director office", number="42.01b", phone='+001111'
        )
...
class AdminTestCase(BaseTestCase):
    ...
    def setUp(self):
        ...
        self._user = UserFactory(
            username='test_user', cleartext_password='password', superuser=True
        )
    ...
    def test_group_add(self):
        response = self.client.post(
            '/admin/examples/ldapgroup/add/', {'gid': '1002', 'name': 'wizgroup'}
        )
    ...
    def test_user_delete(self):
        response = self.client.post(
            '/admin/examples/ldapuser/foouser/delete/', {'yes': 'post'}
        )
